...
public class procces {
    ...
    public void calculateInterest(double rate) {
        if (rate > 0) {
            ...
            bank1.setBalance(bank1.getBalance() + (int) interest);
    ...
    if (amount > salary * 20) {
        return "Failed_amount_too_high";
    }
    if (months < 6 || months > 60) {
        return "Failed_invalid_duration";
    }
    if (bank1.getBalance() < amount * 0.05) {
        return "Failed_low_balance_for_security";
    }
    return "Success";
}
